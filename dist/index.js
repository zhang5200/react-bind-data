...
const MittContext = React__default__default.createContext({ emitter });
const MittProvider = ({ children }) => {
    return (React__default__default.createElement(MittContext.Provider, { value: { emitter } }, children));
};
const useMitt = () => {
    const context = React__default.useContext(MittContext);
...
const Provide = React__default__default.createContext({});
const Provider = (props) => {
    const { children, value } = props;
    return React__default__default.createElement(Provide.Provider, { value: value }, children);
};
const useInject = () => React__default.useContext(Provide);
...
function useCreation(factory, deps) {
    const { current } = React__default.useRef({
        deps,
        obj: undefined,
        initialized: false,
    });
...
const useUpdate = () => {
    ...
    return React__default.useCallback(() => setState({}), []);
};
...
function useReactive(initialState, value) {
    ...
    const stateRef = React__default.useRef(initialState);
...
function useObserver(fn, baseComponentName) {
  ...
  var _a = __read(React__default__default.useState(objectToBeRetainedByReactFactory), 1),
      objectRetainedByReact = _a[0]; // Force update, see #2982


  var _b = __read(React__default__default.useState(), 2),
      setState = _b[1];
  ...
  var reactionTrackingRef = React__default__default.useRef(null);
  ...
  React__default__default.useDebugValue(reaction, printDebugValue);
...
if (!React__default.Component) throw new Error("mobx-react requires React to be available");
